...
export class AccountStore {
  ...
  /** Backward-compatible single upsert. */
  saveCredentials(
    credentials: ClineCredentials,
    options: { label?: string | null; provider?: string } = {},
  ): StoredAccount {
    return this.saveCredentialsWithResult(credentials, options).record;
  }

  /** Single upsert with information the caller needs for import accounting. */
  saveCredentialsWithResult(
    credentials: ClineCredentials,
    options: { label?: string | null; provider?: string } = {},
  ): CredentialSaveResult {
    this.reloadIfChanged();
    const result = this.upsertCredential(credentials, options);
    this.persist();
    return result;
  }

  /** Upsert many credentials with one atomic write for the whole batch. */
  saveCredentialsBatch(items: readonly CredentialSaveInput[]): CredentialSaveResult[] {
    if (items.length === 0) return [];
    this.reloadIfChanged();
    const results = items.map((item) =>
      this.upsertCredential(item.credentials, item.options ?? {}),
    );
    this.persist();
    return results;
  }
  ...
  private upsertCredential(
    ...
    return { record: { ...record }, created: existing === undefined };
  }
