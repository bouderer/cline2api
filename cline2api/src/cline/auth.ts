...
/**
 * Replace the provider selected by the Cline authorize redirect.
 *
 * Cline's authorize endpoint redirects to WorkOS with `provider=authkit` by
 * default. Selecting the real IdP directly avoids the extra AuthKit chooser.
 */
export function withAuthProvider(
  ...
  const normalized = provider.trim() || "MicrosoftOAuth";
  parsed.searchParams.set("provider", normalized);
  return parsed.toString();
}

/**
 * Start the localhost callback flow by following Cline's authorize redirect
 * manually. We do not follow it here because the browser, not the gateway,
 * must complete the interactive login.
 */
export async function startCallbackAuth(
  options: AuthEndpointOptions,
  callbackUrl: string,
): Promise<string> {
  const authorizeUrl = new URL(resolveUrl(options.clineApiBaseUrl, CLINE_AUTH_ENDPOINTS.authorize));
  ...
  if (!isRedirect || !location) {
    ...
    throw new ClineAuthError(
      `Callback authorization failed: ${response.status}${details.message ? ` - ${details.message}` : ""}`,
      {
        status: response.status,
  ...
  let workosUrl: string;
  try {
    workosUrl = new URL(location, authorizeUrl).toString();
  } catch {
    throw new ClineAuthError("Callback authorization returned an invalid redirect", {
      status: response.status,
      ...(response.headers.get("x-request-id")
        ? { requestId: response.headers.get("x-request-id") as string }
        : {}),
  ...
  return withAuthProvider(workosUrl, options.provider ?? "MicrosoftOAuth");
}

export interface AuthorizationCodeResult extends DevicePollResult {
  /** Present when /auth/token already returned fully registered Cline tokens. */
...
/** Step 2 (callback flow): exchange the localhost authorization code. */
export async function exchangeAuthorizationCode(
  code: string,
  callbackUrl: string,
  options: AuthEndpointOptions,
): Promise<AuthorizationCodeResult> {
  const response = await fetch(resolveUrl(options.clineApiBaseUrl, CLINE_AUTH_ENDPOINTS.token), {
    method: "POST",
    headers: {
  ...
  if (text) {
    try {
      ...
      if (response.ok) throw new Error("Invalid token exchange response");
    }
  }

  if (!response.ok) {
    const details = parseAuthError(text);
    throw new ClineAuthError(
  ...
  const refreshToken = stringField(data, "refresh_token") ?? stringField(data, "refreshToken");
  const tokenType =
    stringField(data, "token_type") ?? stringField(data, "tokenType") ?? "Bearer";
  if (accessToken && refreshToken) {
    return { accessToken, refreshToken, tokenType };
  }

  // Some Cline versions make this endpoint perform the registration step and
  // return the already-registered Cline envelope. Accept both shapes so the
  // gateway remains compatible while still registering raw WorkOS tokens.
  if (payload.success === true && isRecord(payload.data)) {
    const credentials = toClineCredentials(
      payload.data as unknown as ClineTokenResponseData,
      options.provider,
    ...
    return {
      ...
      tokenType: String(credentials.metadata?.tokenType ?? "Bearer"),
      credentials,
    };
  }
